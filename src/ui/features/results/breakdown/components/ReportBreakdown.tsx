// src/ui/features/results/breakdown/components/ReportBreakdown.tsx
...
import { Chip, ToggleGroup } from "@/ui/design-system";
import type { TableDensity } from "@/ui/shared";
import { EmptyFilterRow } from "../../components/EmptyFilterRow";
import { breakdownCellPadding, sortIcon, toAriaSort, zebraRowClass } from "../../components/tablePresentation";
import { CopyFeedbackOverlay } from "./CopyFeedbackOverlay";
import { useBreakdownRows, type PageBreakdownRow, type VisualBreakdownRow } from "../hooks/useBreakdownRows";
import { useBreakdownState } from "../hooks/useBreakdownState";
import { useCopyFeedback } from "../hooks/useCopyFeedback";
import { iconToggleButtonClass } from "../utils/iconToggleButton";
...
export function ReportBreakdown({
	...
	const sectionSpacingClass = isCompact ? "space-y-1 pt-1.5" : "space-y-2 pt-2";
	const headerPaddingClass = isCompact ? "py-1" : "py-1.5";
	...
	return (
		<section className={`ml-2 border-t border-(--app-stroke) ${sectionSpacingClass}`}>
			<div className={`grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 px-2 ${headerPaddingClass}`}>
				...
				<div className="justify-self-center">
					<div className="inline-flex flex-col items-start leading-tight">
						<p className="text-[11px] font-semibold text-(--app-link)">{summaryRow.table}</p>
					...
					<div className="flex items-center gap-1">
						<div className="relative">
							...
							<input
								...
								className="h-7 w-48 rounded border border-(--app-stroke) bg-(--app-surface-0) pl-7 pr-7 text-xs text-(--app-fg-primary) placeholder:text-(--app-fg-muted) focus:outline-none focus:ring-2 focus:ring-(--app-focus-ring)"
							/>
							{query ? (
								<button
									type="button"
									onClick={() => setQuery("")}
									className="absolute right-2 top-1/2 -translate-y-1/2 text-(--app-fg-muted) transition-colors hover:text-(--app-link)"
						...
						<button
							...
							className={iconToggleButtonClass(searchLocked)}
						>
...
function PagesTable({
				...
				cell: (info) => {
					...
					return (
						<button
							...
							className="inline-flex cursor-pointer items-center rounded-sm px-1 text-(--app-link) underline-offset-2 transition-colors hover:bg-(--app-fill-hover) hover:text-(--app-link-hover) hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--app-focus-ring)"
						>
	...
	return (
		<div className="overflow-hidden rounded-md border border-(--app-stroke) bg-(--app-surface-1)">
			<div className="overflow-auto">
				<table className="w-full border-collapse text-xs">
					<thead>
						{table.getHeaderGroups().map((headerGroup) => (
							<tr key={headerGroup.id} className="bg-(--app-surface-1)">
								{headerGroup.headers.map((header) => {
									...
									return (
										<th
											...
											className={`border border-(--app-stroke) p-0 font-semibold text-(--app-fg-primary) ${alignment}`}
										>
											{header.isPlaceholder ? null : (
												<button
													type="button"
													onClick={header.column.getToggleSortingHandler()}
													className={`group flex h-full w-full select-text items-center gap-1 ${cellPaddingClass} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--app-focus-ring) ${justify}`}
		...
		cols.push(
				...
				cell: (info) => {
					...
					return (
						<div className="flex w-full min-w-0 items-center justify-between gap-1">
							...
							<button
								type="button"
								className="inline-flex h-4 w-4 shrink-0 cursor-pointer items-center justify-center rounded text-(--app-fg-muted) opacity-0 transition hover:text-(--app-link) group-hover:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--app-focus-ring)"
			...
			},
			{
				...
				cell: (info) => {
					...
					return (
						<span title="Hidden" className="pbix-hidden-check inline-flex items-center justify-center">
									...
									{headerGroup.headers.map((header) => {
										...
										return (
											<th
												...
												scope="col"
												aria-sort={toAriaSort(sorted)}
												className={`border border-(--app-stroke) p-0 font-semibold text-(--app-fg-primary) ${alignment} ${widthClass}`}
											>
												{header.isPlaceholder ? null : canSort ? (
													<button
														type="button"
														onClick={header.column.getToggleSortingHandler()}
														className={`group flex h-full w-full select-text items-center gap-1 ${cellPaddingClass} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--app-focus-ring) ${
													...
													<span className={`block ${cellPaddingClass}`}>
