		...
		<div>
			<div data-testid="amount-breakdown">
				<ReportBreakdown
					...
					gridBorderClass="border-(--app-stroke)"
				/>
			</div>
...
describe("ReportBreakdown", () => {
	...
	it("renders Pages by default and switches to Visuals", async () => {
		...
		expect(tableNameHeader.className).toContain("text-(--app-link)");
	...
	it("renders hidden indicator and role chips in Visuals mode", async () => {
		...
		expect(valuesChip?.className ?? "").toContain("bg-(--app-surface-0)");
		expect(hiddenChip?.className ?? "").toContain("pbix-hidden-check");
