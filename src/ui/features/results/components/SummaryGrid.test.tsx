...
function renderGrid({
	...
	onApplySummaryFilter?: ReturnType<typeof vi.fn>;
} = {}) {
	return render(
		<SummaryGrid
			...
			onApplySummaryFilter={onApplySummaryFilter}
		/>,
	);
}
...
describe("SummaryGrid", () => {
	...
	it("renders expansion row when expanded and toggles on row double-click only", () => {
		...
		const onApplySummaryFilter = vi.fn();
		renderGrid({
			onToggleRow,
		...
		expect(onApplySummaryFilter).toHaveBeenCalledWith("Orders");

		fireEvent.doubleClick(screen.getByRole("button", { name: "Filter summary by table Orders" }));
