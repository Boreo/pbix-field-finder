...
describe("FilesStrip", () => {
	it("accepts dropped files and reports unsupported files", () => {
		...
		expect(strip.className).toContain("bg-(--app-surface-1)");
		expect(strip.className).toContain("border-(--app-stroke-subtle)");
		...
		expect(strip.className).toContain("ring-(--app-accent-blue)");
	...
	it("shows multi-file mode with Add files, Clear all, and file header", () => {
		...
		expect(chip?.className ?? "").toContain(
			"border-[color-mix(in_srgb,var(--color-ctp-green)_52%,var(--app-stroke))]",
		);
