// src/ui/features/ingest/components/FilesStrip.tsx
import { useRef, type ChangeEvent } from "react";
import { Eye, EyeOff, Trash2, Upload, X } from "lucide-react";
import { Button, Chip, IconButton, Panel } from "@/ui/design-system";
...
export function FilesStrip({
	...
	return (
		<Panel
			className={cn(
				"rounded-lg border-(--app-stroke-subtle) bg-(--app-surface-1) shadow-none transition-colors",
				dragActive
					? "bg-(--app-fill-hover) ring-1 ring-(--app-accent-blue)"
					: "bg-(--app-surface-1)",
			)}
						...
						{files.map((item) => (
							<Chip
								...
								className={cn(
									...
									item.errorMessage
										? "border-ctp-red bg-[color-mix(in_srgb,var(--color-ctp-red)_14%,var(--app-surface-0))] text-(--app-fg-danger)"
										: "border-[color-mix(in_srgb,var(--color-ctp-green)_52%,var(--app-stroke))] bg-(--app-surface-0)",
								)}
							>
