...
describe("AboutSection", () => {
	it("renders prominent styling when requested", () => {
		...
		expect(section.className).toContain("border-[color-mix(in_srgb,var(--app-accent-secondary)_32%,var(--app-stroke)_68%)]");
		expect(section.className).not.toContain("border-(--app-stroke)");
	});
	...
	it("renders subtle styling and uses theme-specific github logo", () => {
		...
		const section = screen.getByTestId("about-section");
		expect(section.className).toContain("border-(--app-stroke)");
